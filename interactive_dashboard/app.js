...
// ⛵ 全象徵物航海道具產生器 (Symbolic Nautical Items System)
function getNodeIconSVG(node, isDone, isStanding) {
  // 1. 終點耀眼燈塔島 (Grid 100 Victory Summit)
  if (node.type === "destination") {
    return `
      <div class="relative w-full h-full flex items-center justify-center">
        <div class="island-lagoon-glow" style="background: radial-gradient(ellipse at center, rgba(251, 191, 36, 0.6) 0%, transparent 75%);"></div>
        <div class="water-ripple-ring" style="border-color: rgba(251, 191, 36, 0.8);"></div>
        <div class="island-deep-shadow"></div>
        <img src="images/island_lighthouse.png?v=900" alt="Lighthouse Victory Island" class="natural-island-img w-full h-full object-contain relative z-10">
        <div class="underwater-submersion-mask z-15"></div>
      </div>
    `;
  }

  // 2. ⚔️ 社交練習挑戰據點 ➔ 全新 2.5D 雙劍交叉 (Crossed Pirate Cutlasses)
  if (node.type === "challenge" || node.type === "achievement") {
    return `
      <div class="relative w-full h-full flex items-center justify-center">
        <div class="absolute inset-0 bg-red-500/25 rounded-full filter blur-md animate-pulse"></div>
        <img src="images/crossed_swords.png?v=400" alt="Crossed Pirate Swords Challenge" class="w-full h-full object-contain relative z-10 filter drop-shadow-[0_8px_16px_rgba(239,68,68,0.6)]">
      </div>
    `;
  }

  // 3. 🍾 靈感/故事獎勵據點 ➔ 發光瓶中信 (Message Drift Bottle)
  if (node.type === "reward") {
    return `
      <div class="relative w-full h-full flex items-center justify-center">
        <div class="absolute inset-0 bg-emerald-400/30 rounded-full filter blur-md animate-pulse"></div>
        <img src="images/bottle_inspiration.png?v=500" alt="Inspiration Drift Bottle" class="w-full h-full object-contain relative z-10 filter drop-shadow-[0_8px_16px_rgba(52,211,153,0.6)]">
      </div>
    `;
  }
  ...
  if (node.type === "start") {
    return `
      <div class="relative w-full h-full flex items-center justify-center">
        <div class="absolute inset-0 bg-amber-400/30 rounded-full filter blur-md animate-pulse"></div>
        <img src="images/challenge_node.png?v=200" alt="Start Anchor" class="w-full h-full object-contain relative z-10 filter drop-shadow-[0_8px_16px_rgba(245,158,11,0.6)]">
      </div>
    `;
  }

  // 5. 普通關卡格子 ➔ 統一採用溫暖木質船錨 (Wooden Anchor)
  return `
    <div class="relative w-full h-full flex items-center justify-center">
      <div class="absolute inset-0 bg-amber-400/15 rounded-full filter blur-sm"></div>
      <img src="images/challenge_node.png?v=200" alt="Nautical Wooden Anchor" class="w-full h-full object-contain relative z-10 filter drop-shadow-[0_6px_12px_rgba(245,158,11,0.5)]">
    </div>
  `;
}
